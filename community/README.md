...
#### 1.2 发送成功后刷新私信列表*
### 2，设置已读
#### 2.1 访问私信详情时，将显示的私信设置为已读状态
## 十三，统一异常处理
### 1，@ControllerAdvice 修饰类，表明该类是Controller全局配置类，可以做异常处理方案，绑定数据方案，绑定参数方案
### 2，@ExceptionHandler 修饰方法，在Controller出现异常后被调用，用于处理捕获到的异常
### 3，@ModelAttribute 用于修饰方法，该方法会在Controller方法执行前被调用，用于为Model对象绑定参数
### 4, @DataBinder 用于修饰方法，该方法会在Controller方法执行前被调用，用于绑定参数的转换器
## 十四，统一记录日志
### 1，针对帖子模块，评论模块，消息模块记录日志
### 2，AOP面向切面编程 
#### 2.1 AspectJ在编译期织入代码
#### 2.2 SpringAOP在运行时织入代码（代理）
#### 2.3 代理（，）
##### 2.3.1 JDK动态代理，需要有接口
##### 2.3.2 CGLib动态代理，通过子类创建代理对象
## 十五，Redis，一站式高性能存储方案
### 1，Redis会将内存中的数据以快照（体积小，但是慢，可能产生阻塞）或者日志（存储命令，体积大）的形式保存到硬盘上，保证数据的安全性
### 2，应用场景：缓存，排行榜，计数器，社交网络，消息队列等操作比较频繁的功能。
## 十六，Spring整合Redis
### 1，引入依赖
### 2，配置Redis
#### 2.1 配置数据库参数
#### 2.2 配置编写类，构造RedisTemplate（重新配置Redis组件）
#### 2.3 访问Redis
## 十七，点赞
### 1，支持对帖子，评论点赞，第一次点赞，第二次取消
### 2，统计帖子的点赞数量和详情等
## 十八，关注
### 1，关注和取消关注功能
### 2，统计用户的关注数、粉丝数
## 十九，优化登录模块
### 1，将验证码从session转存到Redis中
### 2，将登录凭证的对象从MySQL转存到Redis中
### 3，将用户信息在Redis中做一下缓存
## 十九，消息队列发送系统级的消息
### 1，阻塞队列
#### 1.1 BlockingQueue
...
##### 1.3.3 PriorityBlockingQueue、SynchronousQueue、DeplayQueue
## 二十，kafka
### 1，简介
#### 1.1 分布式的流媒体平台
#### 1.2 应用：消息系统，日志收集，用户行为追踪，流失处理
### 2，特点
#### 2.1 高吞吐量，消息持久化（将消息存到硬盘上），高可靠性，高扩展性
### 3，术语
#### 3.1 kafka术语：Broker，Zookeeper，Topic，Partition，Offset，Leader Replica，Follow Replica
## 二十一，Spring整合kafka
### 1，配置kafka（server，consumer）
### 2，访问kafka
#### 2.1 生产者：kafkaTemplate类
#### 2.2 消费者：@kafkaListener注解
## 二十二，发送系统通知
### 1，触发事件
#### 1.1 评论后发布通知
#### 1.2 点赞后发布通知
...
#### 2.2 开发事件的生产者
#### 2.3 开发事件的消费者
## 二十三，Elasticsearch搜索引擎
### 1，一根分布式、Restful风格的搜索yinqing
### 2，支持对各种类型的数据的检索
### 3，搜索速度快，可以提供实时的搜索服务
